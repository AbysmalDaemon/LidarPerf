...
- `step13_kiss_hilti_report.html` and `step13_kiss_hilti_report.json` are the first durable `lidarperf report` outputs. They summarize the verified Step 10 real bundle and attach the Step 11 semantic-comparison context. The report preserves the hosted result's `exploratory` / non-authoritative status and makes no new performance claim.
